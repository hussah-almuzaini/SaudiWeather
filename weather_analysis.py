...
with filter_cols[0]:
    ...
    map_type = st.selectbox("", ["Temperature", "Humidity", "Dew Point", "Wind Speed"], key="map_type")
min_date = df["date"].min()
max_date = df["date"].max()
with filter_cols[1]:
    st.markdown("<h5 style='color:#5d9c7d; margin-bottom: 5px;'>📅 Start Date</h5>", unsafe_allow_html=True)
    start_date = st.date_input("", value=min_date, min_value=min_date, max_value=max_date, key="start_date")

with filter_cols[2]:
    st.markdown("<h5 style='color:#5d9c7d; margin-bottom: 5px;'>📅 End Date</h5>", unsafe_allow_html=True)
    end_date = st.date_input("", value=max_date, min_value=min_date, max_value=max_date, key="end_date")
